<html>
<head>
  <?php include_once("head.php"); ?>
  <title>Generation On</title>  
</head>
<body>
	<div id="wrapper">
	<?php include_once("header.php"); ?>
	
  <!-- Banner Start -->
  <?php include_once("banner-inner-center.php"); ?>
  
  <!-- Content Start -->
  <div id="content">
  	<section class="light-md-grey track-service-section track-service-1">
        <div class="container">
          <div class="track-service-center content-center-part center-block white-bg panel-shadow">
            <div class="track-service-inner">
              <div class="track-steps text-center">
                <ul class="clearfix">
                  <li>
                    <div class="step-number">
                      <span>1</span>
                    </div>
                    <span>Project Info</span>
                  </li>
                  <li>
                    <div class="step-number">
                      <span>2</span>
                    </div>
                    <span>Impact</span>
                  </li>
                  <li>
                    <div class="step-number">
                      <span>3</span>
                    </div>
                    <span>Story</span>
                  </li>
                  <li class="active">
                    <div class="step-number">
                      <span>4</span>
                    </div>
                    <span>Participation</span>
                  </li>
                </ul>
              </div>
              <div class="track-body">
                    <div class="track-body-grey light-md-grey"></div>
                    <div class="track-body-content">
                      <h2 class="h2 h2-sub">Club Participation</h2>
                      <form>
                        <div class="bordered-field-block">
                            <div class="option-block">
                              <p>Track Club Roster Participants</p>
                              <div class="radio-group">
                                <div class="radio-inline style-radio purple-radio">
                                  <label>
                                    <input type="radio" name="radio-1" checked>Yes
                                    <span class="check-style">
                                      <span></span>
                                    </span>
                                  </label>
                                </div>
                                <div class="radio-inline style-radio purple-radio">
                                  <label>
                                    <input type="radio" name="radio-1">No
                                    <span class="check-style">
                                      <span></span>
                                    </span>
                                  </label>
                                </div>
                              </div>
                            </div>
                          </div>
                          <div class="bordered-field-block">
                            <div class="table-responsive">
                              <table class="table table-striped switch-table">
                                <thead>
                                  <tr>
                                    <th><span class="text-uppercase">NAME</span></th>
                                    <th><span class="text-uppercase">ATTENDANCE</span></th>
                                  </tr>
                                </thead>
                                <tbody>
                                  <tr>
                                    <td class="name-td">Shelly Liu</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">David Rossberg</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox">
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Markus Bems</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Vera Williamson</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Markus Bems</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Vera Williamson</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Markus Bems</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Vera Williamson</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Markus Bems</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                  <tr>
                                    <td class="name-td">Vera Williamson</td>
                                    <td>
                                      <div class="field-switch">
                                        <span class="switch-label label-left">Attended</span>
                                        <div class="swich-input">
                                          <input type="checkbox" name="my-checkbox" checked>
                                        </div>
                                        <span class="switch-label label-right">Absent</span>
                                      </div>
                                    </td>
                                  </tr>
                                </tbody>
                              </table>
                            </div>  
                          </div>
                      </form>
                    </div>
              </div>
              <div class="track-footer clearfix">
                <a href="track-service-3.php" class="btn btn-grey btn-outline pull-left">BACK</a>
                <a href="#" class="btn btn-blue pull-right">Done</a>
              </div>
            </div>
          </div>
        </div>
    </section>
  </div>

	<?php include_once("footer.php"); ?>
</div><!--wrapper -->
</body>
</html>